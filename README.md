...
| 🖨️ **6 寸相纸智能排版** | 自动比较横竖两种排版，选出张数最多的方案，一页排满不浪费 |
| 📄 **PDF 切割线导出** | 输出真实物理尺寸的 PDF，附灰色点状切割线，裁剪整齐不跑偏 |
| 📏 **8 种常用规格** | 一寸 / 小一寸 / 大一寸（护照）/ 二寸 / 小二寸 / 大二寸 / 身份证 / 驾驶证 |
...
3. 按菜单选择证件照尺寸（一寸、护照、身份证……）
4. 按菜单选择背景颜色（白色 / 蓝色 / 红色 / 浅蓝色）
5. 稍等片刻，在当前目录得到三个文件：

| 文件 | 用途 |
| --- | --- |
| `single_<尺寸名>.jpg` | 单张证件照，用于网上报名、电子证件 |
| `layout_6inch_<尺寸名>.jpg` | 6 寸相纸排版图，可直接发给打印店 |
| `layout_6inch_<尺寸名>_切割线.pdf` | **推荐打印使用**，真实物理尺寸 + 切割线 |
...
photolayout/
  cli.py                命令行交互和输出文件命名
  config.py             照片规格、背景色和纸张配置
  layout.py             尺寸缩放、相纸排版、PDF 导出
  models.py             共享数据模型
  portrait.py           人脸、关键点、裁剪和增强算法
